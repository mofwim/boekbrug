...
//
// Buying from a business abroad shifts the btw to YOU: rubriek 4b for a supplier in another member
// state, 4a for one outside the EU, and the deductible share back in 5b. This file used to LIST the
// EU suppliers it could see through the prefix of their btw-nummer and compute nothing. The
// computation — and the per-invoice listing that goes with it — lives in foreign-purchase-vat.ts
// now ([BUITENLANDSE-INKOOP]); it reads the country the owner recorded on the supplier first and the
// prefix here (classifyVatNumber) second, so nothing this file could see is lost.
