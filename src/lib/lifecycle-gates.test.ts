...
// ── [BEL-BEREIKT-NIEMAND] Every notification goes through the one writer that can deliver it ──
//
// src/lib/notifications.ts has called itself "the canonical notification writer" since the day it
// was made. Measured: 30 of the app's 39 notification writes went straight to the table, and the
// split was not random — the direct ones are the URGENT ones, because urgency gets written where
// the event happens: "Laatste aanmaning verstuurd", "Laatste aanmaning mogelijk NIET verstuurd",
// "Controleer deze betaling", "Terugkerende factuur staat klaar", "Factuur niet verzonden".
//
// Bypassing cost two things, and neither was visible from any screen:
//
//   · NO PUSH. Only that file calls sendPushToUser. A row written directly appears in the app and
//     never on the owner's phone — so an owner who turned push on to be told when something needs
//     them was, for 30 of 39 messages, told nothing until they happened to open the app.
...
test("[BEL-BEREIKT-NIEMAND] nothing writes the notifications table except its one writer", () => {
  const CANON = "src/lib/notifications.ts";
  const walk = (dir: string): string[] => {
    const out: string[] = [];
    for (const e of readdirSync(dir)) {
      const p = `${dir}/${e}`;
  ...
  const offenders: string[] = [];
  for (const file of walk("src")) {
    if (file === CANON || file.includes(".test.")) continue;
    const src = code(file);
    if (/from\(["']notifications["']\)\s*\.\s*(insert|upsert)\(/.test(src)) offenders.push(file);
  }
  assert.deepEqual(
    offenders, [],
    "these write the notifications table directly, so their message never reaches a phone and a " +
      "failed write leaves no trace:\n" + offenders.map((o) => `  · ${o}`).join("\n") +
      `\n\nUse notifyRow / notifyRows / createNotification from ${CANON}. notifyRow takes the ROW ` +
      "shape on purpose — moving a call site changes the call, never the Dutch inside it.",
  );

  // The writer earns being the only one: it reads the error AND delivers the push.
  const canon = code(CANON);
  assert.match(
    canon, /const \{ error \} = await pipeline\.from\('notifications'\)\.insert\(rows\)/,
    "the one writer must read its error — this is the whole reason the others may not write here",
  );
  assert.match(canon, /if \(error\) \{\s*[\s\S]{0,200}?console\.error\(/, "…and say so");
  assert.match(canon, /return false/, "…and report it, so a caller can tell a bell that rang from one that did not");
  assert.match(
    canon, /sendPushToUser\(r\.user_id/,
    "…and push every row it writes, which is what the 30 bypassing sites were silently missing",
  );
  // The push may never decide whether the notification counted: a phone that is offline is not a
  // failure to notify, and the row is already the record.
  assert.match(
    canon, /await Promise\.allSettled\(/,
    "one device's push failure may not take down the others, nor the answer",
  );
});
