...
export const TOOLS: PublicTool[] = [
  // [TOOLS-ORDER] The order is the ranking. This is a flat grid with no
  // headings, so position is the only thing saying what matters — and
  // otherTools() rotates the cross-links by index, so it decides what every
  // tool page points at as well.
  //
  // Sorted by INTENT, not by search volume. "pdf samenvoegen" is looked up far
  // more often than "uurtarief berekenen", and the second is worth more: one is
  // somebody with a document, the other is somebody doing their books. The file
  // tools were inserted at the top when they landed, which pushed all five
  // calculators below sixteen of them — this puts that back.
  {
    slug: '/factuur-maken',
    emoji: '🧾',
    title: 'Factuur maken',
