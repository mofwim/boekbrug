...
export async function POST(request: NextRequest) {
  try {
    ...
    const { invoiceId, convertOnly = false } = body
    // convertOnly=true: "Maak factuur aan" flow — convert pro_forma to factuur without re-sending email
    if (!invoiceId) {
    ...
    // ── 5. Status check ────────────────────────────────────────
    // convertOnly: allow sent pro_formas (Maak factuur aan flow)
    // normal send: only drafts allowed
    ...
    const finalType: string = isConversion ? 'factuur' : (invoice.invoice_type ?? 'factuur')
    ...
    // ── 8. UPDATE DB — commit number + type (legal trigger) ───
    ...
    // ── 11. Send email — skipped for convertOnly (already sent before) ─
