                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, flexShrink: 0 }}>
                      ...
                      {isOfferte && inv.status === 'sent' && (
                        <button
                          onClick={async e => {
                            ...
                            try {
                              const res = await fetch('/api/invoice/send', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ invoiceId: inv.id, convertOnly: true }),
                              })
                              if (!res.ok) {
                                const err = await res.json().catch(() => ({}))
                                showToast(err.error || 'Omzetten mislukt')
                              } else {
                                const result = await res.json().catch(() => ({}))
                                updateOptimistic(inv.id, {
                                  ...(result.invoice_number ? { invoice_number: result.invoice_number } : {}),
                                  invoice_type: 'factuur',
                                })
                                showToast(`Factuur ${result.invoice_number} aangemaakt ✓`)
                              }
                            } catch {
                              showToast('Omzetten mislukt — controleer je verbinding')
                            } finally {
                              setProcessingId(null)
                            }
                          }}
                          ...
                          {processingId === inv.id ? '...' : 'Maak factuur aan'}
                        </button>
                      )}
                    </div>
                  </div>
