  ...
  return (
    <div style={s.card}>
      ...
      <div style={s.toggleRow}>
        <div>
          <span style={{ fontSize: 15, color: '#202124', fontWeight: 500 }}>{t.bufferLabel}</span>
          {useBuffer && (
            <input
              value={buffer}
      ...
      <div style={{ marginTop: 18 }}>
        <div style={s.breakdownRow}>
          <span style={{ color: '#5f6368' }}>{t.baseRate}</span>
          <span style={{ fontWeight: 600, color: '#202124' }}>{fmt(base)}</span>
        </div>
        {useBuffer && (
          <div style={s.breakdownRow}>
            <span style={{ color: '#5f6368' }}>{t.buffer} ({bufferPct}%)</span>
            <span style={{ fontWeight: 600, color: '#202124' }}>{fmt(recommended - base)}</span>
          </div>
        )}
        <div style={{ ...s.breakdownRow, borderBottom: 'none' }}>
          <span style={{ color: '#5f6368' }}>{t.yearRevenue}</span>
          <span style={{ fontWeight: 600, color: '#202124' }}>{fmt(yearRevenue)}</span>
        </div>
      </div>
      ...
      <div style={{ marginTop: 22, background: '#f8f9fa', border: '1px solid #e0e0e0', borderRadius: 14, padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
        ...
        <Link href={t.ctaHref} style={{ backgroundColor: '#1a73e8', color: '#fff', fontSize: 14, fontWeight: 600, padding: '10px 18px', borderRadius: 9999, textDecoration: 'none', whiteSpace: 'nowrap' }}>
          {t.ctaBtn}
        </Link>
      </div>
    </div>
